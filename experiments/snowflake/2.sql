...
WITH PontosDeTeste (ponto) AS (
    SELECT * FROM (VALUES
        ...
        (ST_MAKEPOINT(-46.87, -23.38)),
        (ST_MAKEPOINT(-47.93, -23.49)),
        (ST_MAKEPOINT(-47.06, -22.90)),
        (ST_MAKEPOINT(-51.37, -20.77)),
        (ST_MAKEPOINT(-46.65, -23.56)),
        (ST_MAKEPOINT(-46.70, -23.52)),
        (ST_MAKEPOINT(-46.75, -23.48)),
        (ST_MAKEPOINT(-46.80, -23.44)),
        (ST_MAKEPOINT(-46.85, -23.40))
    ) AS v(ponto)
)
-- Cenário Desnormalizado
SELECT p.ponto, t.CD_SETOR, t.NM_MUN 
FROM PontosDeTeste p 
JOIN cenario_nao_normalizado t ON ST_CONTAINS(t.geometry, p.ponto);

-- Cenário Normalizado
SELECT p.ponto, t.CD_SETOR, d.NM_MUN 
FROM PontosDeTeste p 
