...
WITH MunicipioAlvo AS (
    SELECT GEOMETRY::UnionAggregate(geometry) AS geom_mun 
...
WHERE t.geometry.STIntersects(m.geom_mun) = 1
GROUP BY t.NM_SITUACAO;

-- Cenário Normalizado
WITH MunicipioAlvo AS (
    SELECT GEOMETRY::UnionAggregate(f.geometry) AS geom_mun
...
, MunicipioAlvo m
WHERE f.geometry.STIntersects(m.geom_mun) = 1
