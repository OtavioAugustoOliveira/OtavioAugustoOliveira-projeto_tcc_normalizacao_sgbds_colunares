...
def carregar_configuracoes(arquivo_config='environment_keys.json'):
    """Carrega as configurações do arquivo JSON"""
    try:
        # Tenta diferentes caminhos para encontrar o arquivo
        caminhos_possiveis = [
            arquivo_config,  # Caminho relativo atual
            os.path.join(os.path.dirname(__file__), arquivo_config),  # Mesmo diretório do config_loader
        ]
        
        for caminho in caminhos_possiveis:
            try:
                with open(caminho, 'r', encoding='utf-8') as f:
                    print(f"✅ Arquivo de configuração encontrado em: {caminho}")
                    return json.load(f)
            except FileNotFoundError:
                continue
        
        print(f"❌ ERRO: Arquivo de configuração não encontrado em nenhum dos caminhos:")
        for caminho in caminhos_possiveis:
            print(f"   - {caminho}")
        return {}
